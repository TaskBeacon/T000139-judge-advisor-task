...
Revision request:
Keep exactly the same two-row content, words, timeline order, numeric values and blank input fields.
Fix only these issues:
1. Make the entire background, especially top18percent empty header, pure flat white. Remove all gradients, glow, shadows and colored external borders. Participant boxes stay pale gray; row labels use restrained flat blue/teal.
2. Remove the yellow warning triangle and both loop icons. Keep the inter-pass boundary as simple bold dark text on white, with thin gray separators. No icons. Keep all offsets and timing labels.
Do not add new conditions, phases, stimuli, people, devices, logos or decorative scenery. Leave the header totally blank.
